...
import { getHmrRefreshHash, getResumeDataCache, workUnitAsyncStorage, getDraftModeProviderForCacheScope, getCacheSignal, isHmrRefresh, getServerComponentsHmrCache } from '../app-render/work-unit-async-storage.external';
import { applyOwnerStack, makeDevtoolsIOAwarePromise, makeDynamicHangingPromise, makeRuntimeHangingPromise, makeStageHangingPromise, makeUntrackedHangingPromise, RENDER_STAGES_BY_DATA_KIND } from '../dynamic-rendering-utils';
...
import { MIN_PRERENDERABLE_EXPIRE, MIN_PREFETCHABLE_STALE, MIN_SHELL_STALE } from './constants';
import { NEXT_CACHE_ROOT_PARAM_TAG_ID } from '../../lib/constants';
import { getCacheHandler, getDevTieredCacheHandler, getPrivateCacheHandler, isBuiltInCacheHandler, isCustomCacheHandler, isMemoryCacheDisabled } from './handlers';
...
import { NestedDynamicUseCacheError, UseCacheDeadlockError, UseCacheTimeoutError } from './use-cache-errors';
...
function generateCacheEntryWithRestoredWorkStore(workStore, cacheContext, clientReferenceManifest, encodedArguments, fn, timeoutError, deadlockError) {
    ...
    return workAsyncStorage.run(workStore, generateCacheEntryWithCacheContext, workStore, cacheContext, clientReferenceManifest, encodedArguments, fn, timeoutError, deadlockError);
}
    ...
    if (cacheContext.kind === 'private') {
        ...
        switch(cacheContext.outerWorkUnitStore.type){
            case 'cache':
                ...
                if (cacheContext.dynamicNestedCacheError !== undefined && (metadata.revalidate === 0 || metadata.expire < MIN_PRERENDERABLE_EXPIRE)) {
...
async function collectResult(savedStream, workStore, cacheContext, innerCacheStore, startTime, errors) {
    ...
    const collectedExpire = forceDynamicCacheLifeInDev ? MIN_PRERENDERABLE_EXPIRE : innerCacheStore.explicitExpire !== undefined ? innerCacheStore.explicitExpire : innerCacheStore.expire;
        ...
        case 'prerender':
            {
                ...
                if (didTimeout) {
                    ...
                    const hangingPromise = makeRuntimeHangingPromise(outerWorkUnitStore.renderSignal, workStore.route, 'dynamic "use cache"', outerWorkUnitStore);
            ...
            if (process.env.__NEXT_DEV_SERVER && outerWorkUnitStore.cacheSignal) {
                ...
                if (stageAtReadStart !== RenderStage.Dynamic) {
                    ...
                    stream = renderToReadableStream(resultPromise, clientReferenceManifest.clientModules, {
                        ...
                        onError (error) {
                            if (devRenderAbortController.signal.aborted && devRenderAbortController.signal.reason === error && error instanceof Error) {
                                ...
                                // the outer handler.
                                const digest = createDigestWithErrorCode(error, stringHash(error.message + (error.stack || '')).toString());
                    ...
                    stream = setupProbeScheduler({
                        ...
                        onProbeCompleted () {
                            const error = deadlockError ?? Object.defineProperty(new InvariantError('`deadlockError` should be constructed inside `cache()` before reaching the probe scheduler.'), "__NEXT_ERROR_CODE", {
                                value: "E1182",
                                enumerable: false,
                                configurable: true
                            });
...
export async function cache(kind, id, boundArgsLength, originalFn, args) {
        ...
        case 'prerender-runtime':
            {
                if (workUnitStore.renderSignal.aborted) {
                    ...
                    return makeUntrackedHangingPromise(workUnitStore.renderSignal, workStore.route, '"use cache" called after prerender ended');
                }
                break;
            }
        case 'prerender-ppr':
        ...
        if (isPrivate) {
            ...
            if (process.env.__NEXT_DEV_SERVER && isCustomCacheHandler(kind)) {
                ...
                if (!tieredCacheHandler) {
                    throw Object.defineProperty(new InvariantError(`Expected a dev tiered cache handler for kind "${kind}".`), "__NEXT_ERROR_CODE", {
                        value: "E1365",
                        enumerable: false,
                        configurable: true
                    });
    ...
    if (isPrivate) {
        ...
        switch(workUnitStore.type){
            ...
            case 'prerender':
                ...
                return makeRuntimeHangingPromise(workUnitStore.renderSignal, workStore.route, expression, workUnitStore);
            case 'prerender-ppr':
            ...
            case 'prerender-runtime':
                {
                    ...
                    const stagedRendering = outerWorkUnitStore.stagedRendering;
                    if (stagedRendering) {
                        await stagedRendering.waitForStage(RENDER_STAGES_BY_DATA_KIND.sessionData);
                    }
                    break;
                }
            case 'request':
                {
                    if (process.env.NODE_ENV === 'development') {
                        ...
                        await makeDevtoolsIOAwarePromise(undefined, outerWorkUnitStore, RENDER_STAGES_BY_DATA_KIND.sessionData);
                    }
                    break;
                }
            case 'private-cache':
        ...
        case 'prerender':
            if (!isPageOrLayoutSegmentFunction) {
                ...
                if (dynamicAccessAbortController.signal.aborted) {
                    ...
                    // data.
                    return makeRuntimeHangingPromise(workUnitStore.renderSignal, workStore.route, 'dynamic "use cache"', workUnitStore);
                }
                break;
            }
        // fallthrough
    ...
    if (process.env.__NEXT_DEV_SERVER && workStore.useCacheProbeMode) {
        ...
        if (result.type === 'prerender-dynamic') {
            ...
            throw Object.defineProperty(new InvariantError('Unexpected `prerender-dynamic` result in `use cache` probe mode.'), "__NEXT_ERROR_CODE", {
                value: "E1183",
                enumerable: false,
                configurable: true
            });
    ...
    if (resumeDataCache) {
        ...
        if (rdcEntry !== undefined) {
            ...
            if (rdcResult !== undefined) {
                if (rdcResult.entry.revalidate === 0 || rdcResult.entry.expire < MIN_PRERENDERABLE_EXPIRE) {
                    ...
                    const defaultCacheLife = workStore.cacheLifeProfiles.default;
                    const shouldReportNestedCacheError = rdcResult.dynamicNestedCacheError !== undefined && defaultCacheLife.revalidate !== 0 && defaultCacheLife.expire >= MIN_PRERENDERABLE_EXPIRE;
                    switch(workUnitStore.type){
                        case 'prerender':
                        ...
                        case 'request':
                            {
                                if (process.env.NODE_ENV === 'development') {
                                    ...
                                    if (rdcResult.entry.expire < MIN_PRERENDERABLE_EXPIRE && rdcResult.hasExplicitExpire === false && shouldReportNestedCacheError) {
                                        throw wrapAsInvalidDynamicUsageError(Object.defineProperty(new Error(nestedCacheShortExpireErrorMessage, {
                                            cause: rdcResult.dynamicNestedCacheError
                                        }), "__NEXT_ERROR_CODE", {
                                            value: "E1244",
                                    ...
                                    await makeDevtoolsIOAwarePromise(undefined, workUnitStore, RENDER_STAGES_BY_DATA_KIND.sessionData);
                                }
                                break;
                            }
                        case 'prerender-ppr':
                        ...
                        case 'prerender-runtime':
                            {
                                ...
                                stagedRendering !== null && stagedRendering.finalStage !== null && stagedRendering.finalStage < postShellStage) {
                                    ...
                                    if (isPrefetchable) {
                                        ...
                                        return makeStageHangingPromise(prerenderStore.renderSignal, workStore.route, 'dynamic "use cache"', prerenderStore);
                                    }
                                    // An unprefetchable entry (stale < MIN_PREFETCHABLE_STALE) is
                                    // excluded from runtime prerenders too.
                                    return makeDynamicHangingPromise(prerenderStore.renderSignal, workStore.route, 'dynamic "use cache"');
                                }
    ...
    if (stream === undefined) {
        ...
        const completedCacheInvocations = isPrivate || !isBuiltInCacheHandler(kind) ? workStore.completedCacheInvocations ??= new Map() : undefined;
        ...
        try {
            ...
            while(stream === undefined){
                ...
                if (entry !== undefined && (entry.revalidate === 0 || entry.expire < MIN_PRERENDERABLE_EXPIRE)) {
                    switch(workUnitStore.type){
                        case 'prerender':
                            ...
                            // 'prerender-runtime' case below serves it — so a runtime
                            // prefetch would include this content.
                            const hangingPromise = makeRuntimeHangingPromise(workUnitStore.renderSignal, workStore.route, 'dynamic "use cache"', workUnitStore);
                ...
                currentTime > entry.timestamp + (process.env.__NEXT_DEV_SERVER ? Math.max(entry.expire, MIN_PRERENDERABLE_EXPIRE) : entry.expire) * 1000 || workStore.isStaticGeneration && currentTime > entry.timestamp + entry.revalidate * 1000) {
                    ...
                    if (!shouldTriggerBackgroundRevalidation && process.env.__NEXT_DEV_SERVER && (entry.expire < MIN_PRERENDERABLE_EXPIRE || isMemoryCacheDisabled() && !isCustomCacheHandler(kind))) {
                        switch(workUnitStore.type){
                            case 'request':
