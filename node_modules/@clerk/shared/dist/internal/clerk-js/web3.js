...
const require_internal_clerk_js_hex = require('./hex.js');
const require_internal_clerk_js_injectedWeb3EthProviders = require('./injectedWeb3EthProviders.js');
const require_internal_clerk_js_injectedWeb3SolanaProviders = require('./injectedWeb3SolanaProviders.js');
...
function createWeb3(moduleManager) {
	...
	const generateWeb3Signature = async (params) => {
		...
		return await wallet.request({
			...
			params: [`0x${require_internal_clerk_js_hex.toHex(nonce)}`, identifier]
		});
	};
	...
	async function getWeb3Wallet(provider, walletName) {
		...
		if (provider === "solana") {
			...
			return await require_internal_clerk_js_injectedWeb3SolanaProviders.getInjectedWeb3SolanaProviders().get(walletName);
		}
		return require_internal_clerk_js_injectedWeb3EthProviders.getInjectedWeb3EthProviders().get(provider);
	}
	return {
		getWeb3Identifier,
